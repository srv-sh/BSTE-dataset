...
class datagen():
    ...
    def gen_srnet_data_with_background(self):
        ...
        while True:
            ...
            print(text1," & ",text2)
            self.bg_list = [k for k in self.bg_list if '.jpg' in k]
            ...
            surf_gt, bbs_gt = render_text_mask.render_text(font, text2, param)
            ...
            surf_gt =render_text_mask.perspective(surf_gt, rotate, zoom, shear, perspect, padding)
            ...
            surf_gt = render_text_mask.center2size(surf_gt, (surf_h, surf_w))
            ...
            # augment surf
            surfs = [[surf1, surf2, surf_gt]]
            self.surf_augmentor.augmentor_images = surfs
            surf1, surf2 , surf_gt = self.surf_augmentor.sample(1)[0]
            ...
            _, g_t = colorize.colorize(surf_gt, t_b, fg_col, bg_col, self.colorsRGB, self.colorsLAB, min_h, param)
        ...
        return [i_t, i_s, t_sk, t_t, t_b, t_f, surf2,g_t]
...
class multiprocess_datagen():
    ...
    def dequeue_batch(self, batch_size, data_shape):
        ...
        i_t_batch, i_s_batch, g_t_batch = [], []
        t_sk_batch, t_t_batch, t_b_batch, t_f_batch = [], [], [], []
        mask_t_batch = []
        
        for i in range(batch_size):
            i_t, i_s, t_sk, t_t, t_b, t_f, mask_t,g_t = self.dequeue_data()
        ...
        for i in range(batch_size): 
            ...
            g_t_batch[i] = cv2.resize(g_t_batch[i], to_size)
        ...
        g_t_batch = g_t_batch.astype(np.float32) / 127.5 - 1.
        mask_t_batch = mask_t_batch.astype(np.float32) / 255.
        
        return [i_t_batch, i_s_batch, t_sk_batch, t_t_batch, t_b_batch, t_f_batch, mask_t_batch,g_t_batch]
