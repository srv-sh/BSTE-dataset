...
def main():
    ...
    g_t_dir = os.path.join(cfg.data_dir, cfg.g_t_dir)
    ...
    for idx in range(cfg.sample_num):
        ...
        g = datagen()
        i_t, i_s, t_sk, t_t, t_b, t_f, mask_t,g_t = g.gen_srnet_data_with_background()
        # print(g_t.shape)
        # input("!!!!")
        try:
          i_t, i_s, t_sk, t_t, t_b, t_f, mask_t,g_t = g.gen_srnet_data_with_background()
          ...
          g_t_path = os.path.join(g_t_dir, str(idx).zfill(digit_num) + '.png')
          ...
          cv2.imwrite(g_t_path, g_t, [int(cv2.IMWRITE_PNG_COMPRESSION), 0])
